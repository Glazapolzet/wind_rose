...
def make_report(
    ...
    try:
        ...
        if date_from >= date_to:
            return (
                False,
                f"Ошибка: дата 'с' ({date_from}) должна быть раньше даты 'по' ({date_to})",
            )
        ...
        try:
            ...
            return (
                False,
                f"Ошибка при загрузке данных из файла '{os.path.basename(station.csv_path)}': {str(e)}",
            )
        ...
        if data.empty:
            ...
            if conditions:
                ...
                return (
                    False,
                    f"Нет данных, удовлетворяющих условиям ({cond_str}) за период с {period}",
                )
            else:
                period = f"{date_from.strftime('%d.%m.%Y')} по {date_to.strftime('%d.%m.%Y')}"
        ...
        try:
            ...
            if not os.path.exists(doc_name):
                return (
                    False,
                    f"Ошибка: файл отчета не был сохранен: {os.path.basename(doc_name)}",
                )
        except Exception as e:
...
def _draw_and_save_wind_rose_plot(data, rose_name, save_to, total_count=None):
    ...
    if len(data) == 0:
        raise ValueError(
            "Нет данных для построения розы ветров после преобразования направлений"
        )
